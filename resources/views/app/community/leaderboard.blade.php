    ...
    <section class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
                ...
                <div class="rounded-[28px] border border-white/[0.08] bg-[#111214] p-6">
                    ...
                    <div class="mt-5 space-y-3">
                        @foreach($leaderboard[$key] as $index => $user)
                            <div class="flex items-center justify-between gap-3 rounded-2xl border border-white/[0.06] bg-black/20 p-4">
                                <div class="flex min-w-0 items-center gap-3">
                                    <span class="shrink-0 text-sm text-[#A3A3A3]">#{{ $index + 1 }}</span>
                                    <span class="truncate font-semibold text-white">{{ $user->name }}</span>
                                </div>
                                <span class="shrink-0 text-sm text-[#A3A3A3]">{{ $user->reviews_count ?? $user->review_comments_count ?? $user->audio_files_count ?? $user->helpful_total ?? 0 }}</span>
                            </div>
                        @endforeach
                    </div>
                </div>
        ...
        @if($leaderboard['highestRatedReview'])
            <div class="mt-5 rounded-[28px] border border-white/[0.08] bg-[#111214] p-6">
                ...
                <x-community.rating-stars :rating="$leaderboard['highestRatedReview']->rating" size="16" class="mt-2 whitespace-nowrap" />
            </div>
        @endif
    </section>
@endsection
